...
serve(async (req: Request) => {
  ...
  try {
    const { plant, userMessage, tonePersona, insights } = await req.json();
    console.log('[EDGE][generate-plant-response] Payload recibido:', { plant, userMessage, tonePersona, insights });

    if (!plant || !userMessage) {
      console.log('[EDGE][generate-plant-response] Faltan plant o userMessage en el payload');
    ...
    console.log('[EDGE][generate-plant-response] Calculando mood...');
    const mood = calculatePlantMood(plant);
    console.log('[EDGE][generate-plant-response] Mood calculado:', mood);

    // Construir sección de insights si existen
    let insightsSection = '';
    if (insights && Array.isArray(insights) && insights.length > 0) {
      insightsSection = '\nINSIGHTS DE EVOLUCIÓN Y SALUD (útiles para responder si son relevantes):\n' +
        insights.map((ins: any) => `- [${ins.type}] ${ins.title}: ${ins.message}`).join('\n');
    }

    console.log('[EDGE][generate-plant-response] Construyendo systemPrompt...');
    let systemPrompt = getSystemPrompt(plant, mood, tonePersona);
    // Inserta los insights justo después de la sección de imágenes
    if (insightsSection) {
      // Busca el final de la sección de imágenes para insertar los insights
      const imagesSectionEnd = systemPrompt.indexOf('IMÁGENES DE LA PLANTA');
      if (imagesSectionEnd !== -1) {
        // Encuentra el salto de línea después de la sección de imágenes
        const afterImages = systemPrompt.indexOf('\n', imagesSectionEnd);
        if (afterImages !== -1) {
          systemPrompt = systemPrompt.slice(0, afterImages + 1) + insightsSection + '\n' + systemPrompt.slice(afterImages + 1);
        } else {
          systemPrompt += insightsSection + '\n';
        }
      } else {
        systemPrompt += insightsSection + '\n';
      }
    }
    // Añade instrucción explícita para que la IA use los insights si son relevantes
    systemPrompt += '\nSI HAY INSIGHTS, ÚSALOS PARA RESPONDER SI SON RELEVANTES A LA PREGUNTA DEL USUARIO.';
    console.log('[EDGE][generate-plant-response] systemPrompt construido:', systemPrompt.slice(0, 500) + '...');

    console.log('[EDGE][generate-plant-response] Llamando a OpenAI...');
    ...
    console.log('[EDGE][generate-plant-response] Respuesta de OpenAI recibida:', response);

    const content = response.choices[0]?.message?.content;
    if (!content) {
      console.log('[EDGE][generate-plant-response] No se recibió contenido de OpenAI');
    ...
    console.error('[EDGE][generate-plant-response] Error generando respuesta:', error, error?.stack);
